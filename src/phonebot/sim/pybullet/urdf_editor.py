...
# [GCC 5.4.0 20160609]
# Embedded file name: /home/jamiecho/Repos/PhoneBot/control/sim/gazebo/urdfEditor.py
# Compiled at: 2019-11-18 06:07:02
# Size of source mod 2**32: 31295 bytes
...
class UrdfEditor(object):
    ...
    def convertLinkFromMultiBody(self, bodyUid, linkIndex, urdfLink, physicsClientId):
        ...
        visualShapes = p.getVisualShapeData(
            bodyUid, physicsClientId=physicsClientId)
        matIndex = 0
    ...
    def initializeFromBulletBody(self, bodyUid, physicsClientId):
        ...
        baseLink = UrdfLink()
        baseLinkIndex = -1
        self.convertLinkFromMultiBody(
            bodyUid, baseLinkIndex, baseLink, physicsClientId)
        baseLink.link_name = p.getBodyInfo(bodyUid, physicsClientId=physicsClientId)[
            0].decode('utf-8')
        ...
        for j in range(p.getNumJoints(bodyUid, physicsClientId=physicsClientId)):
            jointInfo = p.getJointInfo(
                bodyUid, j, physicsClientId=physicsClientId)
            urdfLink = UrdfLink()
            self.convertLinkFromMultiBody(
                bodyUid, j, urdfLink, physicsClientId)
            ...
            dynChild = p.getDynamicsInfo(
                bodyUid, j, physicsClientId=physicsClientId)
            childInertiaPos = dynChild[3]
            childInertiaOrn = dynChild[4]
            parentCom2JointPos = jointInfo[14]
            parentCom2JointOrn = jointInfo[15]
            tmpPos, tmpOrn = p.multiplyTransforms(
                childInertiaPos, childInertiaOrn, parentCom2JointPos, parentCom2JointOrn)
            tmpPosInv, tmpOrnInv = p.invertTransform(tmpPos, tmpOrn)
            dynParent = p.getDynamicsInfo(
                bodyUid, orgParentIndex, physicsClientId=physicsClientId)
            parentInertiaPos = dynParent[3]
            parentInertiaOrn = dynParent[4]
            pos, orn = p.multiplyTransforms(
                parentInertiaPos, parentInertiaOrn, tmpPosInv, tmpOrnInv)
            pos, orn_unused = p.multiplyTransforms(
                parentInertiaPos, parentInertiaOrn, parentCom2JointPos, [0, 0, 0, 1])
            urdfJoint.joint_origin_xyz = pos
            urdfJoint.joint_origin_rpy = p.getEulerFromQuaternion(orn)
            self.urdfJoints.append(urdfJoint)

    def writeInertial(self, file, urdfInertial, precision=None):
        if precision is None:
            precision = self.precision
        file.write('\t\t<inertial>\n')
        str = '\t\t\t<origin rpy="{:.{prec}f} {:.{prec}f} {:.{prec}f}" xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
            urdfInertial.origin_rpy[0], urdfInertial.origin_rpy[1], urdfInertial.origin_rpy[2], urdfInertial.origin_xyz[0], urdfInertial.origin_xyz[1], urdfInertial.origin_xyz[2], prec=precision)
        file.write(str)
        str = '\t\t\t<mass value="{:.{prec}f}"/>\n'.format(
            urdfInertial.mass, prec=precision)
        file.write(str)
        str = '\t\t\t<inertia ixx="{:.{prec}f}" ixy="0" ixz="0" iyy="{:.{prec}f}" iyz="0" izz="{:.{prec}f}"/>\n'.format(
            urdfInertial.inertia_xxyyzz[0], urdfInertial.inertia_xxyyzz[1], urdfInertial.inertia_xxyyzz[2], prec=precision)
        file.write(str)
        file.write('\t\t</inertial>\n')

    def writeVisualShape(self, file, urdfVisual, precision=None):
        if precision is None:
            precision = self.precision
        ...
        file.write('\t\t<visual>\n')
        str = '\t\t\t<origin rpy="{:.{prec}f} {:.{prec}f} {:.{prec}f}" xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
            urdfVisual.origin_rpy[0], urdfVisual.origin_rpy[1], urdfVisual.origin_rpy[2], urdfVisual.origin_xyz[0], urdfVisual.origin_xyz[1], urdfVisual.origin_xyz[2], prec=precision)
        file.write(str)
        file.write('\t\t\t<geometry>\n')
        if urdfVisual.geom_type == p.GEOM_BOX:
            str = '\t\t\t\t<box size="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
                urdfVisual.geom_extents[0], urdfVisual.geom_extents[1], urdfVisual.geom_extents[2], prec=precision)
            file.write(str)
        if urdfVisual.geom_type == p.GEOM_SPHERE:
            str = '\t\t\t\t<sphere radius="{:.{prec}f}"/>\n'.format(
                urdfVisual.geom_radius, prec=precision)
            file.write(str)
        if urdfVisual.geom_type == p.GEOM_MESH:
            str = '\t\t\t\t<mesh filename="{}" scale="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
                urdfVisual.geom_meshfilename, urdfVisual.geom_meshscale[0], urdfVisual.geom_meshscale[1], urdfVisual.geom_meshscale[2], prec=precision)
            file.write(str)
        ...
        str = '\t\t\t\t<color rgba="{:.{prec}f} {:.{prec}f} {:.{prec}f} {:.{prec}f}" />\n'.format(
            urdfVisual.material_rgba[0], urdfVisual.material_rgba[1], urdfVisual.material_rgba[2], urdfVisual.material_rgba[3], prec=precision)
        file.write(str)
        file.write('\t\t\t</material>\n')
        file.write('\t\t</visual>\n')

    def writeCollisionShape(self, file, urdfCollision, precision=None):
        if precision is None:
            precision = self.precision
        file.write('\t\t<collision>\n')
        str = '\t\t\t<origin rpy="{:.{prec}f} {:.{prec}f} {:.{prec}f}" xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
            urdfCollision.origin_rpy[0], urdfCollision.origin_rpy[1], urdfCollision.origin_rpy[2], urdfCollision.origin_xyz[0], urdfCollision.origin_xyz[1], urdfCollision.origin_xyz[2], prec=precision)
        file.write(str)
        file.write('\t\t\t<geometry>\n')
        if urdfCollision.geom_type == p.GEOM_BOX:
            str = '\t\t\t\t<box size="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
                urdfCollision.geom_extents[0], urdfCollision.geom_extents[1], urdfCollision.geom_extents[2], prec=precision)
            file.write(str)
        if urdfCollision.geom_type == p.GEOM_SPHERE:
            str = '\t\t\t\t<sphere radius="{:.{prec}f}"/>\n'.format(
                urdfCollision.geom_radius, prec=precision)
            file.write(str)
        if urdfCollision.geom_type == p.GEOM_MESH:
            str = '\t\t\t\t<mesh filename="{}" scale="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
                urdfCollision.geom_meshfilename, urdfCollision.geom_meshscale[0], urdfCollision.geom_meshscale[1], urdfCollision.geom_meshscale[2], prec=precision)
            file.write(str)
    ...
    def writeJoint(self, file, urdfJoint, precision=None):
        if precision is None:
            precision = self.precision
        ...
        str = '\t<joint name="{}" type="{}">\n'.format(
            urdfJoint.joint_name, jointTypeStr)
        file.write(str)
        ...
        str = '\t\t<origin xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
            urdfJoint.joint_origin_xyz[0], urdfJoint.joint_origin_xyz[1], urdfJoint.joint_origin_xyz[2], prec=precision)
        str = '\t\t<origin rpy="{:.{prec}f} {:.{prec}f} {:.{prec}f}" xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
            urdfJoint.joint_origin_rpy[0], urdfJoint.joint_origin_rpy[1], urdfJoint.joint_origin_rpy[2], urdfJoint.joint_origin_xyz[0], urdfJoint.joint_origin_xyz[1], urdfJoint.joint_origin_xyz[2], prec=precision)
        file.write(str)
        str = '\t\t<axis xyz="{:.{prec}f} {:.{prec}f} {:.{prec}f}"/>\n'.format(
            urdfJoint.joint_axis_xyz[0], urdfJoint.joint_axis_xyz[1], urdfJoint.joint_axis_xyz[2], prec=precision)
        file.write(str)
        file.write('\t</joint>\n')

    def to_string(self, saveVisuals=True):
        import io
        f = io.StringIO()
        self.saveUrdfBase(f, saveVisuals=True)
        return f.getvalue()

    def saveUrdfBase(self, file, saveVisuals=True):
    ...
    def saveUrdf(self, fileName, saveVisuals=True):
        with open(fileName, 'w') as (file):
            self.saveUrdfBase(file, saveVisuals)

    def joinUrdf(self, childEditor, parentLinkIndex=0, jointPivotXYZInParent=[
            0, 0, 0], jointPivotRPYInParent=[
            0, 0, 0], jointPivotXYZInChild=[
            0, 0, 0], jointPivotRPYInChild=[
            0, 0, 0], parentPhysicsClientId=0, childPhysicsClientId=0):
        ...
        pos, orn = p.multiplyTransforms(self.urdfLinks[childLinkIndex].urdf_inertial.origin_xyz, p.getQuaternionFromEuler(
            self.urdfLinks[childLinkIndex].urdf_inertial.origin_rpy), invJointPivotXYZInChild, invJointPivotQuatInChild, physicsClientId=parentPhysicsClientId)
        self.urdfLinks[childLinkIndex].urdf_inertial.origin_xyz = pos
        self.urdfLinks[childLinkIndex].urdf_inertial.origin_rpy = p.getEulerFromQuaternion(
            orn)
        for v in self.urdfLinks[childLinkIndex].urdf_visual_shapes:
            pos, orn = p.multiplyTransforms(v.origin_xyz, p.getQuaternionFromEuler(
                v.origin_rpy), invJointPivotXYZInChild, invJointPivotQuatInChild)
            v.origin_xyz = pos
            v.origin_rpy = p.getEulerFromQuaternion(orn)

        for c in self.urdfLinks[childLinkIndex].urdf_collision_shapes:
            pos, orn = p.multiplyTransforms(c.origin_xyz, p.getQuaternionFromEuler(
                c.origin_rpy), invJointPivotXYZInChild, invJointPivotQuatInChild)
            c.origin_xyz = pos
    ...
    def createMultiBody(self, basePosition=[
            0, 0, 0], baseOrientation=[
        ...
        if len(baseShapeTypeArray):
            baseCollisionShapeIndex = p.createCollisionShapeArray(shapeTypes=baseShapeTypeArray, radii=baseRadiusArray, halfExtents=baseHalfExtentsArray, lengths=lengthsArray, fileNames=fileNameArray,
                                                                  meshScales=meshScaleArray, collisionFramePositions=basePositionsArray, collisionFrameOrientations=baseOrientationsArray, physicsClientId=physicsClientId)
        ...
        visualFrameOrientations = [p.getQuaternionFromEuler(
            v.origin_rpy) for v in urdfVisuals]
        baseVisualShapeIndex = -1
        if len(shapeTypes):
            baseVisualShapeIndex = p.createVisualShapeArray(shapeTypes=shapeTypes, halfExtents=halfExtents, radii=radii, lengths=lengths, fileNames=fileNames, meshScales=meshScales,
                                                            rgbaColors=rgbaColors, visualFramePositions=visualFramePositions, visualFrameOrientations=visualFrameOrientations, physicsClientId=physicsClientId)
        linkMasses = []
        ...
        for joint in self.urdfJoints:
            ...
            if len(linkShapeTypeArray):
                linkCollisionShapeIndex = p.createCollisionShapeArray(shapeTypes=linkShapeTypeArray, radii=linkRadiusArray, halfExtents=linkHalfExtentsArray, lengths=lengthsArray, fileNames=fileNameArray,
                                                                      meshScales=linkMeshScaleArray, collisionFramePositions=linkPositionsArray, collisionFrameOrientations=linkOrientationsArray, physicsClientId=physicsClientId)
            ...
            if len(shapeTypes):
                ...
                linkVisualShapeIndex = p.createVisualShapeArray(shapeTypes=shapeTypes, halfExtents=halfExtents, radii=radii, lengths=lengths, fileNames=fileNames, meshScales=meshScales,
                                                                rgbaColors=rgbaColors, visualFramePositions=visualFramePositions, visualFrameOrientations=visualFrameOrientations, physicsClientId=physicsClientId)
        ...
        obUid = p.createMultiBody(baseMass, baseCollisionShapeIndex=baseCollisionShapeIndex, baseVisualShapeIndex=baseVisualShapeIndex, basePosition=basePosition, baseOrientation=baseOrientation, baseInertialFramePosition=base.urdf_inertial.origin_xyz, baseInertialFrameOrientation=p.getQuaternionFromEuler(base.urdf_inertial.origin_rpy), linkMasses=linkMasses, linkCollisionShapeIndices=linkCollisionShapeIndices,
...
def export_urdf(robot_id, sim_id, builder, filename='/tmp/robot.urdf'):
    # Build robot model.
    urdf = UrdfEditor()
    urdf.initializeFromBulletBody(robot_id, sim_id)
    text = urdf.to_string()
    # Remap link/joint names
    sremap = {'link0': 'body'}
    for i in reversed(range(p.getNumJoints(robot_id))):
